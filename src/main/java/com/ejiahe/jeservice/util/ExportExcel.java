...
import com.ejiahe.jeservice.util.test.Book;
import com.ejiahe.jeservice.util.test.Student;
...
/**
 * 导出excel工具
 * Created by liaofei on 2018/1/15.
 */
public class ExportExcel<T>{
    ...
    public void exportExcel(String title, Collection<T> dataset, OutputStream out){
        exportExcel(title, null, dataset, out, YYYY_MM_DD_HH_MM_SS);
    }

    public void exportExcel(String title, String[] headers, Collection<T> dataset,
                            OutputStream out){
        exportExcel(title, headers, dataset, out, YYYY_MM_DD_HH_MM_SS);
    }
...
//        font2.setBoldweight(HSSFFont.BOLDWEIGHT_NORMAL);
        ...
        while (it.hasNext())
        {
            ...
            //TODO 从第二个字段开始处理（为了忽略放第一位的id属性），所以设置i=1， 但有些别扭， 后续可考虑更优雅的方案
            for (short i = 1; i < fields.length; i++){
                //i-1 表示excel单元格还是应该从0开始计算
                ...
                try
                {
                    ...
                    else if (value instanceof byte[] && value != null)
                    {
                        ...
                        sheet.setColumnWidth(i-1, (short) (35.7 * 80));//TODO i->i-1
    ...
    public static void main(String[] args)
    {
        ...
        try
        {
            BufferedInputStream bis = new BufferedInputStream(
                    new FileInputStream("F://image/qie.jpg"));
            ...
            ex.exportExcel("测试POI导出EXCEL文档", headers, dataset, out);
            ex2.exportExcel("测试POI导出EXCEL文档", headers2, dataset2, out2);
//            out.close();
//            out2.close();
